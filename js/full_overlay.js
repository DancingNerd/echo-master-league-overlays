...
const team_logo_background_orange = document.getElementById(
  "team_logo_background_orange"
);
...
const team_logo_background_blue = document.getElementById(
  "team_logo_background_blue"
);
...
const team_logos_collapsible = document.getElementsByClassName(
  "team_logos_collapsible"
);
const team_names_collapsible = document.getElementsByClassName(
  "team_names_collapsible"
);
...
// math time
const mathematical_time_box = document.getElementById(
  "mathematical_time_content"
);
...
const orange_goal_player_name = document.getElementById(
  "orange_goal_player_name"
);
const orange_goal_assist_name = document.getElementById(
  "orange_goal_assist_name"
);
const orange_goal_point_value = document.getElementById(
  "orange_goal_point_value"
);
...
const orange_goal_banner_text = document.getElementById(
  "orange_goal_banner_text"
);
const blue_goal_banner_text = document.getElementById("blue_goal_banner_text");
const orange_goal_banner_secondary = document.getElementById(
  "orange_goal_banner_secondary"
);
const blue_goal_banner_secondary = document.getElementById(
  "blue_goal_banner_secondary"
);
const orange_goal_banner_point_value = document.getElementById(
  "orange_goal_banner_point_value"
);
const blue_goal_banner_point_value = document.getElementById(
  "blue_goal_banner_point_value"
);

const event_log_container = document.getElementById("event_log_container");

const iframe_compact_main_banner = document.getElementById(
  "iframe_compact_main_banner"
);
const iframe_default_minimap = document.getElementById(
  "iframe_default_minimap"
);
const iframe_compact_minimap = document.getElementById(
  "iframe_compact_minimap"
);
...
if (orange_joust !== null && blue_joust !== null) {
  sw.subscribe("joust", (data) => {
    if (data["team_color"] === "orange") {
      ...
      orange_joust.innerText =
        Math.round(data["joust_time"] * 100) / 100 + " s";
...
sw.subscribe("overlay_config", (data) => {
  ...
  if (team_logo_blue !== null) {
    ...
    if (data["teams"][1]["team_logo"] === "") {
      team_logo_orange.src =
        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=";
    }
  }
  if (team_logo_background_blue !== null) {
    team_logo_background_blue.style.backgroundImage =
      "url(" + data["teams"][0]["team_logo"] + ")";
    team_logo_background_orange.style.backgroundImage =
      "url(" + data["teams"][1]["team_logo"] + ")";
  }
...
function UpdateMinimap(session) {
  ...
  for (let i = 0; i < 10; i++) {
    if (
      session["teams"][Math.floor(i / 5)]["players"] &&
      session["teams"][Math.floor(i / 5)]["players"].length > i % 5
    ) {
      const player_data =
        session["teams"][Math.floor(i / 5)]["players"][Math.floor(i % 5)];
      set_pos(
        players[i],
        player_data["head"]["position"][2],
        player_data["head"]["position"][0]
      );
      set_number(player_numbers[i], " " + player_join_number(i));
...
function UpdateMathematicalTime(data, lastData) {
  ...
  let mathematicalTime =
    game_clock - (Math.ceil(Math.abs(orange_score - blue_score) / 3) * 20 - 20);
...
function UpdatePlayerLists(data, lastData) {
  ...
  let newBluePlayerList = Array.from(
    bluePlayers,
    (p) => p["name"] + p["possession"] + p["stunned"]
  ).join("");
  let newOrangePlayerList = Array.from(
    orangePlayers,
    (p) => p["name"] + p["possession"] + p["stunned"]
  ).join("");
  ...
  function CreatePlayers(playerList, appendList) {
    for (let i = 0; i < playerList.length; i++) {
      ...
      numberDiv.innerText = player_join_number(i);
...
function set_clock() {
  if (clockRunning) {
    ...
    game_clock_display.innerText = `${String(minutes).padStart(
      2,
      "0"
...
function set_pos(elem, z, x) {
  elem.style.left = (-z / 80 + 0.5) * 26.6875 + 4.25 + "em";
  elem.style.top = (x / 32 + 0.5) * 10.3125 + 0.3125 + "em";
}
